jQuery( function ( $ ) {
	function runAjaxAction( button, action ) {
		button = $( button );
		const panel = button.closest( '.puc-debug-bar-panel-v5' );
		const responseBox = button.closest( 'td' ).find( '.puc-ajax-response' );
	...
	const panels = $( '#debug-menu-targets' ).find( '.puc-debug-bar-panel-v5' );
	panels.each( function () {
		const panel = $( this );
		const uid = panel.data( 'uid' );
		const target = panel.closest( '.debug-menu-target' );
